...
int main()
{
	...
	if (pid) {
		...
		for (;;) {
			...
			nfds = epoll_wait(poll_fd, events, 10, 3000);//@events not POed for returning 0(no fd available) or -1(signal rxed);
			evt = events[0].events;
...
#if 10//if removed, then no EPOLL_CTL_MOD and no more return for epoll_wait until child terminates
